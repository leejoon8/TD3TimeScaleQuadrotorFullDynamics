...
class PhysicsSim():
    def __init__(self, init_pose=None, init_velocities=None, init_angle_velocities=None, runtime=5.):
        ...
        #######################################
        self.init_space = \
            spaces.Box(low=np.array([-np.pi / 4.0, -np.pi / 4.0, -np.pi / 4.0,\
                -20, -20, -np.pi / 4.0, -np.pi / 4.0, -np.pi / 4.0 ]), \
                high=np.array([ np.pi / 4.0, np.pi / 4.0, np.pi / 4.0,\
                20, 20, np.pi / 4.0, np.pi / 4.0, np.pi / 4.0]))
        ########################################
    ...
    def reset(self):
        ...
        init_states = np.array([0.0, 0.0, 10.0,\
            init[0], init[1], init[2],\
            init[3], init[4], 0.0,\
                init[5] * np.cos(init[4]), init[6], init[7]* np.cos(init[6])])
        
        #self.pose = np.array([0.0, 0.0, 10.0, 0.0, 0.0, 0.0]) if self.init_pose is None else np.copy(self.init_pose)
        self.pose = np.array([0.0, 0.0, 1.0, \
            init[0], init[1], init[2]]) #if self.init_pose is None else np.copy(self.init_pose)
        self.v = np.array([init[3], init[4], 0.0]) #if self.init_velocities is None else np.copy(self.init_velocities)
        #self.angular_v = np.array([init[5] * np.cos(init[4]), init[6], init[7]* np.cos(init[6])]) # if self.init_angle_velocities is None else np.copy(self.init_angle_velocities)
        self.angular_v = np.array([init[5], init[6], init[7]]) # if self.init_angle_velocities is None else np.copy(self.init_angle_velocities)
        self.linear_accel = np.array([0.0, 0.0, 0.0])
        self.angular_accels = np.array([0.0, 0.0, 0.0])
        self.prop_wind_speed = np.array([0., 0., 0., 0.])
        self.angular_v_inertia = np.array([0., 0., 0.])
        self.done = False
        self.in_bounds = True
    ...
    def get_moments(self, thrusts):
        ...
        omegax = self.angular_v[0]
        omegay = self.angular_v[1]
        omegaz = self.angular_v[2]
        
        dynamic_omega= np.array([(self.Iyy-self.Izz)*omegay*omegaz,
                                 (self.Izz-self.Ixx)*omegax*omegaz,
                                 (self.Ixx-self.Iyy)*omegax*omegay])
        moments = thrust_moment + dynamic_omega - drag_moment # + motor_inertia_moment
        return moments
    ...
    def next_timestep(self, rotor_speeds):
        ...
        position = self.pose[:3] + self.v * self.dt # + 0.5 * self.linear_accel * self.dt**2
        self.v += self.linear_accel * self.dt#-1*np.array([1, 1, 1])
        ...
        omega_x = self.angular_v[0]
        omega_y = self.angular_v[1]
        omega_z = self.angular_v[2]
        angle_phi = self.pose[3]
        angle_theta = self.pose[4]
        angle_psi = self.pose[5]
        
        if (angle_theta < (np.pi/2.0 + 0.00001) ) and (angle_theta > (np.pi/2.0 - 0.00001)):
             angle_theta=np.pi/2+0.0001
        
        #if Ta(angle_theta)<0.00001:
        #     angle_theta=np.pi/2+0.0001
        
        self.angular_v_inertia = np.array([omega_x + omega_y*S(angle_phi)*Ta(angle_theta)
                                           + omega_z*C(angle_phi)*Ta(angle_theta),
                                           omega_y*C(angle_phi) - omega_z*S(angle_phi),
                                           omega_y*(S(angle_phi)/C(angle_theta))
                                           + omega_z*(C(angle_phi)/C(angle_theta))])
        #angles = self.pose[3:] + self.angular_v * self.dt #+ 0.5 * self.angular_accels * self.angular_accels * self.dt ** 2
        angles = self.pose[3:] + self.angular_v_inertia * self.dt #+ 0.5 * self.angular_accels * self.angular_accels * self.dt ** 2
        angles = (angles + 2 * np.pi) % (2 * np.pi)
        self.angular_v = self.angular_v + self.angular_accels * self.dt
